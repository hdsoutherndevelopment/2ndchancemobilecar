  ...
  return (
    ...
    >
      <div className="container-page grid gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-16">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-[13px] font-semibold transition ${
                    active === pin.name
                      ...
                      : "border-white/10 bg-white/[0.02] text-chrome-muted hover:text-white"
                  }`}
                >
                  <MapPin className="h-3.5 w-3.5 text-aqua" strokeWidth={2} />
                  {pin.name}
                </button>
              ))}
            </div>
        ...
        <Reveal delay={0.1}>
          <div className="panel noise relative overflow-hidden p-4">
            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl bg-ink-950">
              <svg viewBox="0 0 100 75" className="absolute inset-0 h-full w-full" aria-hidden="true">
                <defs>
                  <linearGradient id="water" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#0E7C6E" stopOpacity="0.35" />
                    <stop offset="100%" stopColor="#0B4F63" stopOpacity="0.15" />
                  </linearGradient>
                ...
                {/* road network */}
                <g stroke="#2FDCC2" strokeOpacity="0.14" strokeWidth="0.5" fill="none">
                ...
                {/* coverage halo */}
                <circle cx="44" cy="50" r="30" fill="#2FDCC2" fillOpacity="0.045" />
                <circle
                  cx="44"
                  cy="50"
                  r="30"
              ...
              {mapPins.map((pin, i) => {
                ...
                return (
                  ...
                  >
                    {pin.primary ? (
                      <span className="absolute left-1/2 top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 animate-pulse-ring rounded-full bg-aqua/40" />
                    ) : null}
                    <span
                      className={`relative block rounded-full transition-all duration-300 ${
                        pin.primary
                          ? "h-3 w-3 bg-aqua shadow-glow"
                          : isActive
                            ? "h-2.5 w-2.5 bg-aqua"
                            : "h-2 w-2 bg-chrome-muted/70"
                      }`}
                    />
                    <span
                      className={`absolute left-1/2 top-full mt-1.5 -translate-x-1/2 whitespace-nowrap text-[9px] font-bold uppercase tracking-wider transition ${
                        pin.primary || isActive ? "text-white" : "text-chrome-muted/70"
                      }`}
                    >
                      {pin.name}
                    </span>
            ...
            <p className="px-2 py-3 text-center text-[11px] text-chrome-muted">
